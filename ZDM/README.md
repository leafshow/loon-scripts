...
| 签到通知「⚠️ 签名已过期」（`Sign time out of data`） | 正常现象：SMZDM 签到 body 内含 `time+sign` 防重放签名，仅捕获后短时间内有效，定时任务过期后必然拒签。**打开什么值得买 App 点一次签到即可**——捕获脚本会自动刷新签名并完成当日签到 |
| 签到失败（其他错误） | Cookie/Body 过期，重新打开 App 签到页捕获；或接口变更，需抓包核对 |
